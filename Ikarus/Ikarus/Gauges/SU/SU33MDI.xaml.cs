...
{
    /// <summary>
    /// Interaction logic for SU33MDI.xaml
    /// </summary>
    public partial class SU33MDI : UserControl, I_Ikarus
    {
        ...
        public SU33MDI()
        {
            ...
            Hook.Visibility = System.Windows.Visibility.Hidden;
            ...
            Wingfold.Visibility = System.Windows.Visibility.Hidden;
            Airbreak.Visibility = System.Windows.Visibility.Hidden;
        }
            ...
            Dispatcher.BeginInvoke(DispatcherPriority.Normal,
                       (Action)(() =>
                       {
                           try
                           {
                               ...
                               if (vals.Length > 3) { hook = Convert.ToDouble(vals[3], CultureInfo.InvariantCulture); }
                               if (vals.Length > 4) { gearWarningLight = Convert.ToDouble(vals[4], CultureInfo.InvariantCulture); }
                               if (vals.Length > 5) { flaps = Convert.ToDouble(vals[5], CultureInfo.InvariantCulture); }
                               if (vals.Length > 6) { slats = Convert.ToDouble(vals[6], CultureInfo.InvariantCulture); }
                               if (vals.Length > 7) { wingfold = Convert.ToDouble(vals[7], CultureInfo.InvariantCulture); }
                               if (vals.Length > 8) { airbrake = Convert.ToDouble(vals[8], CultureInfo.InvariantCulture); }
                               ...
                               Hook.Visibility = (hook > 0.9) ? System.Windows.Visibility.Visible : System.Windows.Visibility.Hidden;
                               ...
                               Wingfold.Visibility = (wingfold > 0.9) ? System.Windows.Visibility.Visible : System.Windows.Visibility.Hidden;
                               Airbreak.Visibility = (airbrake > 0.9) ? System.Windows.Visibility.Visible : System.Windows.Visibility.Hidden;
                           }
                           catch { return; };
                       }));
        }
